...
filename = "GSR_" +sys.argv[2] + timestr + ".csv"
		...
		with open(filename, 'a') as f:
			...
			try:
				while True:
					...
					print( "Timestamp %5d,\t GSR %4d,\t PPG %4d marker %d" % (timestamp, GSR_ohm, PPG_mv, mrk))
					finaldata = [timestamp, GSR_ohm, PPG_mv, mrk]
					# finaldata = [timestamp, GSR_ohm, PPG_mv]

					
					w.writerow(finaldata)
					# csv_file.write("%5d,%4d,%4d"+mrk+"\n" % (timestamp, GSR_ohm, PPG_mv))
